...
const Home = () => {
  ...
  useEffect(() => {
    const checkServerStatus = async () => {
      try {
        const response = await fetch('https://volley.onrender.com');
        console.log(response);
        if (response.ok) {
          toast.success('Servidor iniciado. ¡Bienvenido!', {
          ...
          toast.info('El servidor se iniciará en 50 segundos.', {
            position: "bottom-right",
            autoClose: 5000,
            hideProgressBar: false,
            closeOnClick: true,
    ...
    if (!lastCheck || (now - lastCheck) > 600000) { // 10 minutos = 600000 ms
      checkServerStatus();
        ...
        <div className="max-w-7xl mx-auto">
          <section className="grid md:grid-cols-2 gap-12 mb-16">
            ...
            <div className={`p-8 rounded-xl shadow-md ${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
              ...
              <Link
                to="/team"
                className={`mt-4 inline-block px-6 py-3 rounded-full ${isDarkMode ? 'bg-purple-600 hover:bg-purple-700' : 'bg-orange-600 hover:bg-orange-600'} text-white font-semibold text-lg transition duration-300`}
              >
